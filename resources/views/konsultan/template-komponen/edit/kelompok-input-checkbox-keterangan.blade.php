            ...
            <div class="col-9">    
                ...
                <option value="">[- Pilih List -]</option>
                    @foreach ($tluListCheckboxes as $id => $list_checkbox)
                        <option value="{{ $list_checkbox }}" {{ $list_checkbox == old('list_data_input_keterangan', $templateKomponenPemeriksaan->list_data_input_keterangan) ? "selected" : "" }}>{{ $list_checkbox }}</option>
                    @endforeach
                </select>          
